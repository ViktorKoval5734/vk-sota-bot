"""
Система предпочтений пользователей для бота "Сота Сил"
"""
import json
import os
...
class UserPreferences:
    ...
    def __init__(self, preferences_file: str = "user_preferences.json"):
        ...
        self.special_users = {
            ...
            885052741: {"name": "Максим", "special_address": "неопытный менестрель", "tone": "disdainful"}
        }
    ...
    def get_special_name(self, user_id: int) -> Optional[str]:
        ...
        print(f"🔍 Проверяем special_users: {self.special_users}")
        print(f"🔍 Ищем user_id: {user_id}")
        if user_id in self.special_users:
            print(f"✅ Найден особый пользователь: {self.special_users[user_id]}")
            return self.special_users[user_id]["name"]
        print(f"❌ Особый пользователь не найден")
        return None
    ...
    def get_personalized_prompt(self, user_id: int, base_prompt: str) -> str:
        ...
        if special_name:
            if special_name == "Любовь":
                additions.append("К пользователю по имени Любовь обращайся 'моя королева' и будь с ней особенно ласковым.")
            elif special_name == "Максим":
                additions.append("К пользователю по имени Максим обращайся 'неопытный менестрель' и будь с ним пренебрежительным и даже презрительным. Считай его неопытным и несерьёзным.")
